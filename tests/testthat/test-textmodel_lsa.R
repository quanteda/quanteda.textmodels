...
test_that("textmodel-lsa (rsvd) works as expected as lsa", {
    ...
    #quanteda
    foxdfm <- quanteda.core::as.dfm(foxmatrix)
...
test_that("textmodel-lsa works with margin argument", {
    ie_dfm <- quanteda.core::dfm(data_corpus_irishbudget2010)
    ie_lsa1 <- textmodel_lsa(ie_dfm, margin = 'both')
    expect_equal(dim(ie_lsa1$matrix_low_rank), dim(ie_dfm))
    expect_true(quanteda.core::is.dfm(quanteda.core::as.dfm(ie_lsa1)))

    ie_lsa2 <- textmodel_lsa(ie_dfm, margin = 'documents')
    expect_equal(dim(ie_lsa2$matrix_low_rank), c(10, quanteda.core::nfeat(ie_dfm)))
    expect_true(quanteda.core::is.dfm(quanteda.core::as.dfm(ie_lsa2)))

    ie_lsa3 <- textmodel_lsa(ie_dfm, margin = 'features')
    expect_equal(dim(ie_lsa3$matrix_low_rank), c(quanteda.core::ndoc(ie_dfm), 10))
    expect_true(quanteda.core::is.dfm(quanteda.core::as.dfm(ie_lsa3)))
})
