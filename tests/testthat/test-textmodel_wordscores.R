...
test_that("test wordscores on LBG data", {
    ...
    expect_equal(names(pr2), quanteda.core::docnames(data_dfm_lbgexample))
...
test_that("test wordscores fitted and predicted", {
    ...
    expect_equal(ws$x, quanteda.core::as.dfm(data_dfm_lbgexample))
    expect_equal(ws$y, y)
...
test_that("predict.textmodel_wordscores with rescaling works with additional reference texts (#1251)", {
    refscores <- rep(NA, quanteda.core::ndoc(data_dfm_lbgexample))
    refscores[quanteda.core::docnames(data_dfm_lbgexample) == "R1"] <- -1
    refscores[quanteda.core::docnames(data_dfm_lbgexample) == "R5"] <- 1
...
test_that("additional quanteda methods", {
    ...
    expect_equal(quanteda.core::ndoc(ws), 6)
    expect_equal(quanteda.core::nfeat(ws), 37)
    expect_equal(quanteda.core::docnames(ws), quanteda.core::docnames(data_dfm_lbgexample))
    expect_equal(quanteda.core::featnames(ws),
                 quanteda.core::featnames(data_dfm_lbgexample))
})

test_that("Works with newdata with different features from the model (#1329)", {
    mt1 <- quanteda.core::dfm(c(text1 = "a b c", text2 = "d e f"))
    mt2 <- quanteda.core::dfm(c(text3 = "a b c", text4 = "e f g"))
...
test_that("textmodel_wordscores does not use NA wordscores scores", {
    ...
    thedfm["V1", "ZJ"] <- 1
    thedfm <- quanteda.core::as.dfm(thedfm)
...
test_that("textmodel_wordscores correctly implements smoothing (#1476)", {
    ...
    ws_smooth1 <- textmodel_wordscores(quanteda.core::dfm_smooth(data_dfm_lbgexample, smoothing = 1),
                                        c(seq(-1.5, 1.5, .75), NA), scale = "linear")
...
test_that("predict.textmodel_wordscores correctly implements smoothing (#1476)", {
    ...
    ws_smooth1 <- textmodel_wordscores(data_dfm_lbgexample, smooth = 1,
                                        c(seq(-1.5, 1.5, .75), NA), scale = "linear")
    ws_smooth1a <- textmodel_wordscores(quanteda.core::dfm_smooth(data_dfm_lbgexample, smoothing = 1),
                                        c(seq(-1.5, 1.5, .75), NA), scale = "linear")
    expect_identical(
        predict(ws_smooth1),
    ...
    expect_identical(
        predict(ws_smooth1a),
        predict(ws_smooth1, newdata = quanteda.core::dfm_smooth(data_dfm_lbgexample, smoothing = 1))
    )
})

test_that("textmodel_wordscores() work with weighted dfm", {
    dfmat <- quanteda.core::dfm_tfidf(data_dfm_lbgexample)
    expect_silent(
