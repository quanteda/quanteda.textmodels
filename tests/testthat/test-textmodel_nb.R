...
nb_dfm <- quanteda.core::dfm(txt, tolower = FALSE)
...
test_that("Works with features with zero probability", {
    mt <- quanteda.core::as.dfm(matrix(c(0, 0, 3, 1, 4, 2), nrow = 2))
...
test_that("types works (#1322)", {
    ...
    expect_identical(names(pr), quanteda.core::docnames(nb_multi_smooth))
    expect_is(pr, "factor")
...
test_that("Bernoulli output matches fastNaiveBayes and naivebayes packages", {
    ...
    xb <- quanteda.core::dfm_weight(nb_dfm, scheme = "boolean")
    y <- nb_class
