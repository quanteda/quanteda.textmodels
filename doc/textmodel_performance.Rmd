...
dfmat <- dfm(quanteda::tokens(data_corpus_LMRD))
...
microbenchmark(
    ...
    fastNaiveBayes = { 
      tmod <- fnb.multinomial(as(dfmat_train, "CsparseMatrix"), y = dfmat_train$polarity, laplace = 1, sparse = TRUE)
      pred <- predict(tmod, newdata = as(dfmat_test, "CsparseMatrix"))
    },
    naivebayes = {
      tmod = multinomial_naive_bayes(as(dfmat_train, "CsparseMatrix"), dfmat_train$polarity, laplace = 1)
      pred <- predict(tmod, newdata = as(dfmat_test, "CsparseMatrix"))
    },
    times = 50
)
...
microbenchmark(
    ...
    fastNaiveBayes = { 
      tmod <- fnb.bernoulli(as(dfmat_train_bern, "CsparseMatrix"), y = dfmat_train$polarity, laplace = 1, sparse = TRUE)
      pred <- predict(tmod, newdata = as(dfmat_test_bern, "CsparseMatrix"))
    },
    naivebayes = {
      tmod = bernoulli_naive_bayes(as(dfmat_train_bern, "CsparseMatrix"), dfmat_train$polarity, laplace = 1)
      pred <- predict(tmod, newdata = as(dfmat_test_bern, "CsparseMatrix"))
    },
    times = 50
)
